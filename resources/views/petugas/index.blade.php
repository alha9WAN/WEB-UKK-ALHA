...
<div class="notification-modal" id="notificationModal">
    <div class="notification-header">
        <h3>
            ...
            Notifikasi
            <span class="notification-badge-count" id="notifCount">{{ $count ?? 0 }}</span>
        </h3>
            ...
            <li class="notification-item {{ !$notif->is_read ? 'unread' : '' }}" data-id="{{ $notif->id }}">
                ...
                <div class="notification-content">
                    ...
                    <div class="notification-time">
                      <i class="far fa-clock"></i> {{ $notif->created_at->diffForHumans() }}                    </div>
                </div>
            </li>
@empty
<li style="display:flex; justify-content:center; align-items:center; padding:20px; color:#888;">
...
@endforelse
    ...
    <div class="notification-footer">
        ...
        {{-- Footer --}}
        @include('petugas.components.footer')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 🔊 AUDIO SETUP
    // =========================
    let lastCount = window.initialNotifCount || 0;
    ...
    // auto unlock (kalau bisa)
    if (sound) {
        sound.play().then(() => {
            sound.pause();
            sound.currentTime = 0;
            audioUnlocked = true;
            console.log('🔓 auto unlocked');
        }).catch(() => {
            console.log('❌ butuh klik untuk unlock');
        });
    }
    ...
    setInterval(() => {
        fetch('/notifications/latest')
            ...
            .then(data => {
                ...
                // bunyi kalau notif baru
                if (data.count > lastCount) {
                    playNotifSound();
                    showToast('🔔 Notifikasi baru!');
                }
                ...
                if (list) {
                    ...
                    if (data.notifications.length === 0) {
                        html = `<li class="notification-empty">Tidak ada notifikasi</li>`;
                    } else {
    ...
    document.addEventListener('click', function (e) {
        if (modal && modal.classList.contains('active')) {
            if (!modal.contains(e.target) && e.target !== notifBtn) {
                modal.classList.remove('active');
            }
        }
    });
